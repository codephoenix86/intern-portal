...
import { sendFriendRequestSchema, respondFriendRequestSchema, followUserSchema, unfollowUserSchema, removeFriendSchema, connectionListQuerySchema, } from "../validators/connection.validator.js";
// ── Friend Requests ──
export const sendFriendRequest = async (req, res) => {
    try {
        ...
        const data = await connectionService.sendFriendRequest(req.user.userId, parsed.data.toUserId);
...
export const respondFriendRequest = async (req, res) => {
    try {
        ...
        const data = await connectionService.respondToFriendRequest(req.user.userId, parsed.data.connectionId, parsed.data.action);
...
export const removeFriend = async (req, res) => {
    try {
        ...
        const data = await connectionService.removeFriend(req.user.userId, parsed.data.toUserId);
        sendSuccess(res, 200, data.message, data);
    }
    catch (error) {
        if (error instanceof AppError) {
...
export const cancelFriendRequest = async (req, res) => {
    try {
        ...
        const data = await connectionService.cancelFriendRequest(req.user.userId, parsed.data.toUserId);
        sendSuccess(res, 200, data.message, data);
    }
    catch (error) {
        if (error instanceof AppError) {
...
export const followUser = async (req, res) => {
    try {
        ...
        const data = await connectionService.followUser(req.user.userId, parsed.data.toUserId);
        sendSuccess(res, 201, data.message, data);
    }
    catch (error) {
        if (error instanceof AppError) {
...
export const unfollowUser = async (req, res) => {
    try {
        ...
        const data = await connectionService.unfollowUser(req.user.userId, parsed.data.toUserId);
        sendSuccess(res, 200, data.message, data);
    }
    catch (error) {
        if (error instanceof AppError) {
...
export const listFollowingRecruiters = async (req, res) => {
    try {
        const parsed = connectionListQuerySchema.safeParse(req.query);
        const raw = parsed.success ? parsed.data : { page: 1, limit: 20 };
        const data = await connectionService.listFollowingRecruiters(req.user.userId, { page: raw.page, limit: raw.limit, q: raw.q });
        sendSuccess(res, 200, "OK", data);
    }
    catch (error) {
...
export const listFollowingMentors = async (req, res) => {
    try {
        const parsed = connectionListQuerySchema.safeParse(req.query);
        const raw = parsed.success ? parsed.data : { page: 1, limit: 20 };
        const data = await connectionService.listFollowingMentors(req.user.userId, { page: raw.page, limit: raw.limit, q: raw.q });
        sendSuccess(res, 200, "OK", data);
    }
    catch (error) {
...
// ── Bulk Status ──
export const getBulkFriendStatuses = async (req, res) => {
    try {
        ...
        const userIds = ids
            ...
            .map((s) => s.trim())
            .filter(Boolean);
        const data = await connectionService.getBulkFriendStatuses(req.user.userId, userIds);
        sendSuccess(res, 200, "OK", data);
    }
    catch (error) {
...
export const getBulkFollowStatuses = async (req, res) => {
    try {
        const ids = req.query["ids"];
        if (!ids || typeof ids !== "string") {
            sendError(res, 400, "ids query param required (comma-separated)");
        ...
        const userIds = ids
            ...
            .map((s) => s.trim())
            .filter(Boolean);
        const data = await connectionService.getBulkFollowStatuses(req.user.userId, userIds);
        sendSuccess(res, 200, "OK", data);
    }
    catch (error) {
