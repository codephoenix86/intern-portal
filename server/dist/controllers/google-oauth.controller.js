...
export const initiateGoogleAuth = (req, res) => {
    try {
        ...
        if (role && !validRoles.includes(role)) {
            sendError(res, 400, "Invalid role. Must be student, mentor, or recruiter.");
            return;
        }
        // Generate authorization URL
        const { url, state, codeVerifier } = googleOAuthService.createAuthorizationURL();
        ...
        res.cookie("google_oauth_code_verifier", codeVerifier, OAUTH_STATE_COOKIE_OPTIONS);
...
export const googleCallback = async (req, res) => {
    try {
        ...
        const role = req.cookies?.["google_oauth_role"];
        ...
        const validRole = ["student", "mentor", "recruiter"].find((r) => r === role);
        const result = await googleOAuthService.handleCallback(code, codeVerifier, validRole ?? null, meta);
        // 10. Set auth cookies
