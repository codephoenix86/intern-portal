...
export const listCatalogCourses = async (req, res) => {
    try {
...
export const getCatalogCourse = async (req, res) => {
    try {
        ...
        const data = await studentCourseService.getCatalogCourse(req.user.userId, courseId);
        sendSuccess(res, 200, "OK", data);
    }
    catch (error) {
...
export const enrollCatalogCourse = async (req, res) => {
    try {
        const courseId = routeParam(req.params["courseId"]);
        if (!courseId) {
            sendError(res, 400, "Course ID required");
        ...
        const data = await studentCourseService.enroll(req.user.userId, courseId);
...
export const unenrollCatalogCourse = async (req, res) => {
    try {
        const courseId = routeParam(req.params["courseId"]);
        if (!courseId) {
            sendError(res, 400, "Course ID required");
...
export const listMyEnrollments = async (req, res) => {
    try {
        const data = await studentCourseService.listMyEnrollments(req.user.userId);
        sendSuccess(res, 200, "OK", data);
    }
    catch (error) {
        console.error(error);
