...
import { generateAccessToken, generateRefreshToken, generateTokenId, getRefreshTokenExpiry, verifyRefreshToken, } from "../utils/token.utils.js";
// ── Service Class ────────────────────────────────────
class AuthService {
    /**
     * Register a new user
    ...
    async login(email, password, meta) {
        ...
        if (!user.password) {
            throw new AppError(401, `This account uses ${user.provider} login. Please use ${user.provider} to sign in.`);
        }
        // 3. Verify password
     ...
     */
    async refreshAccessToken(refreshTokenJWT, meta) {
        ...
        if (!storedToken) {
            ...
            throw new AppError(401, "Refresh token not found. All sessions have been revoked for security.");
        }
        // 3. Check expiry
