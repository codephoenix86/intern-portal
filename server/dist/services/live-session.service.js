...
class LiveSessionService {
    async ensureDemoSessionsForStudents() {
     ...
     */
    async createSession(mentorId, input) {
        ...
        if (input.courseId) {
            ...
            if (!course) {
                throw new AppError(404, "Course not found or you do not own this course");
            }
        }
     ...
     */
    async getMentorSessions(mentorId, query) {
     ...
     */
    async getAvailableSessionsForStudents(query) {
     ...
     */
    async getSessionById(sessionId, mentorId) {
     ...
     * Update a session
     */
    async updateSession(sessionId, mentorId, input) {
        const session = await LiveSession.findOne({
            _id: sessionId,
            mentorId,
        });
     ...
     */
    async completeSession(sessionId, mentorId) {
        const session = await LiveSession.findOne({
            _id: sessionId,
            mentorId,
        });
     ...
     * Delete a session
     */
    async deleteSession(sessionId, mentorId) {
        const session = await LiveSession.findOne({
            _id: sessionId,
            mentorId,
        });
     ...
     */
    async getUpcomingCount(mentorId) {
     ...
     */
    async joinSession(sessionId, studentId, accessCode) {
        ...
        if (session.type === "paid_class") {
            ...
            // Or check if student is enrolled in the course
                ...
                if (!enrollment) {
                    throw new AppError(403, "You must be enrolled in the course or have an access code to join this session");
                }
            }
            else {
     ...
     */
    async notifyStudentsAboutSession(session, mentorId) {
        try {
            // Get mentor name
            ...
            const typeLabel = session.type === "free_demo" ? "Free Demo" : "Live Class";
