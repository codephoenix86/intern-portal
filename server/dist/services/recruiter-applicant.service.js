...
import { applicationStatusToRecruiter, isRecruiterApplicationStatus, recruiterStatusToApplication, } from "../utils/recruiter-status.utils.js";
import { notificationService } from "./notification.service.js";
import { AppError } from "./auth.service.js";
function applicantNumericId(idHex) {
    let h = 0;
...
class RecruiterApplicantService {
    async listApplicants(recruiterId, filters) {
    ...
    mapApplicant(a, jobTitle, st) {
        const seed = a._id.toString();
        const { skillMatch, experienceMatch, educationMatch } = stableMatchBreakdown(a.matchScore, seed);
        const status = applicationStatusToRecruiter(a.status);
        return {
    ...
    async updateApplicationStatus(recruiterId, applicationId, status) {
        ...
        await Application.updateOne({ _id: applicationId }, { status: next });
...
//# sourceMappingURL=recruiter-applicant.service.js.map