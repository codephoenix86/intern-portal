...
import {
  publicMentorParamsSchema,
  publicMentorsListQuerySchema,
} from "../validators/public-mentors.validator.js";
import { publicMentorsService } from "../services/public-mentors.service.js";

export const listPublicMentors = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const parsed = publicMentorsListQuerySchema.safeParse(req.query);
    type PublicMentorsListQuery = z.infer<typeof publicMentorsListQuerySchema>;

    const raw: PublicMentorsListQuery = parsed.success
      ? parsed.data
      : {
          page: 1,
          limit: 20,
