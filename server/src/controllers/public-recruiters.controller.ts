...
import {
  publicRecruiterParamsSchema,
  publicRecruitersListQuerySchema,
} from "../validators/public-recruiters.validator.js";
import { publicRecruitersService } from "../services/public-recruiters.service.js";

export const listPublicRecruiters = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const parsed = publicRecruitersListQuerySchema.safeParse(req.query);
    type PublicRecruitersListQuery = z.infer<
      typeof publicRecruitersListQuerySchema
    >;

    const raw: PublicRecruitersListQuery = parsed.success
      ? parsed.data
      : {
          page: 1,
          limit: 20,
...
export const getPublicRecruiter = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
