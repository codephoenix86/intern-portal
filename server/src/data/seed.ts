...
const timeStr = (): string => `${String(randInt(9, 20)).padStart(2, "0")}:00`;
...
async function seed() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    studentDocs.push({
      ...
      codingProfiles: {
        ...
        portfolio:
          Math.random() > 0.6 ? `https://${firstName.toLowerCase()}.dev` : null,
      },
      resumeUrl: null,
      parsedResume: null,
      profileViews: randInt(0, 200),
      roadmapTasks: [],
      roadmapInterests: pickN(
        ["Web Development", "ML", "DevOps", "Mobile", "Data Science", "Cloud"],
        randInt(1, 3),
      ),
  ...
  for (let i = 0; i < 25; i++) {
    ...
    jobDocs.push({
      ...
      workType: pick(["Remote", "Hybrid", "On-site"] as const),
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const modules = Array.from({ length: randInt(3, 8) }, (_, mi) => ({
      ...
      description:
        "Comprehensive module covering key concepts with practical examples.",
      contentUrl: null,
      contentType: pick(["video", "pdf", "notes"] as const),
    ...
    const course = await Course.create({
      ...
      level: pick(["Beginner", "Intermediate", "Advanced"] as const),
  ...
  for (let i = 0; i < 20; i++) {
    ...
    sessionDocs.push({
      ...
      type: pick(["free_demo", "paid_class"] as const),
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const moduleProgress = course.modules.map(
      (
        mod: { _id?: mongoose.Types.ObjectId; order: number },
        _idx: number,
      ) => ({
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (connectionPairs.has(key1) || connectionPairs.has(key2)) continue;
    ...
    connectionDocs.push({
      ...
      status: i < 15 ? ("accepted" as const) : ("pending" as const),
    });
  }
  ...
  for (const app of applications.slice(0, 15)) {
    ...
    notificationDocs.push({
      ...
      priority:
        app.status === "Offer" ? ("high" as const) : ("medium" as const),
  ...
  const pendingFriendConns = connectionDocs.filter(
    (c) => c.type === "friend" && c.status === "pending",
  );
  ...
  console.log("");
  console.log(
    "   Students:   student1@internportal.com  → student20@internportal.com",
  );
  console.log(
    "   Recruiters: recruiter1@internportal.com → recruiter10@internportal.com",
  );
  console.log(
    "   Mentors:    mentor1@internportal.com    → mentor8@internportal.com",
  );
