...
export interface IModule {
  title: string;
  description: string;
  contentUrl: string | null; // Cloudinary (video/pdf)
  contentType: "video" | "pdf" | "notes" | "link";
  duration: string; // e.g. "45 min"
  order: number;
  isFree: boolean; // Free preview module
}

export interface IPricing {
  amount: number; // 0 = free course
  currency: string;
...
export interface ICourse {
  ...
  duration: string; // Total duration e.g. "6 Weeks"
  skills: string[];
  category: string; // e.g. "Web Development", "DSA", "ML"

  // Content
  modules: IModule[];
  thumbnailUrl: string | null; // Cloudinary
  previewVideoUrl: string | null; // Cloudinary
...
// ── Main Schema ──────────────────────────────────────
const courseSchema = new Schema<ICourse, any, ICourseMethods>(
  {
    mentorId: {
...
courseSchema.pre("save", function () {
  ...
  if (this.pricing && this.pricing.discountPercent > 0) {
    this.pricing.discountedAmount = Math.round(
      this.pricing.amount * (1 - this.pricing.discountPercent / 100),
    );
