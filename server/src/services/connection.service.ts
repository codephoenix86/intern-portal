import { Connection, type ConnectionType } from "../models/connection.model.js";
...
class ConnectionService {
  ...
  async listFriends(
    userId: string,
    input: { page: number; limit: number; q?: string | undefined },
  ) {
  ...
  async listPendingRequests(
    ...
    const items = conns.map((c) => {
      const from = c.fromUser as unknown as {
        _id: { toString: () => string };
        name: string;
        avatar?: string;
        college?: string;
  ...
  private async _listFollowing(
    userId: string,
    role: "recruiter" | "mentor",
    input: { page: number; limit: number; q?: string | undefined },
  ) {
    const skip = (input.page - 1) * input.limit;
