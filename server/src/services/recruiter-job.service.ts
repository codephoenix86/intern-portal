import { Job, type JobWorkType } from "../models/job.model.js";
...
function workTypeFromBody(
  type: "remote" | "onsite" | "hybrid" | JobWorkType,
): JobWorkType {
  if (type === "remote" || type === "Remote") return "Remote";
  if (type === "onsite" || type === "On-site") return "On-site";
  if (type === "hybrid" || type === "Hybrid") return "Hybrid";
  return type as JobWorkType;
}
...
class RecruiterJobService {
    ...
    const rows = await Promise.all(
      jobs.map(async (job) => {
        ...
        return {
          ...
          postedDate: formatRelativeTime(job.createdAt as Date),
          applicants,
  ...
  async updateJob(
    ...
    if (body.type !== undefined) job.workType = workTypeFromBody(body.type);
    if (body.duration !== undefined) job.duration = body.duration;
    if (body.stipend !== undefined) job.stipend = body.stipend;
