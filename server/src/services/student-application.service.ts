...
class StudentApplicationService {
  async listForStudent(studentId: string) {
    const apps = await Application.find({ studentId })
      .sort({ createdAt: -1 })
      .populate<{ jobId: { title: string; company: string } }>(
        "jobId",
        "title company",
      )
      .lean();
    ...
    return {
      applications: apps.map((a) => {
        const job = a.jobId as unknown as { title: string; company: string };
        return {
          id: a._id.toString(),
          internship: job.title,
          company: job.company,
          status: a.status,
          date: (a.createdAt as Date).toISOString().slice(0, 10),
  ...
  async apply(studentId: string, jobId: string) {
