...
class StudentCourseService {
  ...
  async listCatalog(studentId: string, query: CatalogListQuery) {
    ...
    const enrolledSet = new Set(
      enrolledRows.map((e) => e.courseId.toString()),
    );
  ...
  async getCatalogCourse(studentId: string, courseId: string) {
    await this.ensureCatalogSeeded();
  ...
  async enroll(studentId: string, courseId: string) {
    ...
    const moduleProgress = course.modules.map((m) => {
      const sub = m as unknown as { _id?: Types.ObjectId };
      if (!sub._id) {
  ...
  async listMyEnrollments(studentId: string) {
    ...
    const courses = await Course.find({ _id: { $in: courseIds } })
      .select(
        "title shortDescription level duration category skills thumbnailUrl",
      )
      .lean();
